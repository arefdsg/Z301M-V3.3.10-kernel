...
int boe_id;
static int __init check_boe_id(char *p)
{
        if (p) {
                sscanf(p, "%d", &boe_id);
                printk(KERN_ERR "%s:boe_id = %d\n", __func__, boe_id);
        }
        return 0;
}
early_param("boe_id", check_boe_id);

module_param(boe_id, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(PROJ_VERSION, "BOE_ID judgement");
int Read_BOE_ID(void)
{
        printk(KERN_INFO "BOE_ID = 0x%x \n", boe_id);
        return boe_id;
}
EXPORT_SYMBOL(Read_BOE_ID);
#endif
//youlun add --
