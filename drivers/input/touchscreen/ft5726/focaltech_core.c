...
#define FOCAL_BIEL_HD_FW_PATH "BIEL_HD.bin"
#define FOCAL_BIEL_FHD_FW_PATH "BIEL_FHD.bin"
#define FOCAL_BIEL_FHD_KD_FW_PATH "BIEL_FHD_KD.bin"
#define FOCAL_LAIBAO_HD_FW_PATH "LAIBAO_HD.bin"
#define FOCAL_LAIBAO_FHD_FW_PATH "LAIBAO_FHD.bin"
/*
   tp_id_1 lcm_id_0 lcm_id_1   ID 
   BIEL_HD_BOE      0       0       0       0      v
   BIEL_HD_KD       0       0       1       1
   BIEL_FHD_BOE     0       1       0       2      v
   BIEL_FHD_KD      0       1       1       3
   LAIBAO_HD_BOE    1       0       0       4      v
   LAIBAO_HD_KD     1       0       1       5
   LAIBAO_FHD_BOE   1       1       0       6      v
   LAIBAO_FHD_KD    1       1       1       7

   Only Use tp_id_1 & lcm_id_0 to update firmware (BIEL/LAIBAO & HD/FHD)
 */
#define BIEL_HD_BOE		0
#define BIEL_FHD_BOE	2
#define LAIBAO_HD_BOE	4
#define LAIBAO_FHD_BOE	6
#define BIEL_HD_KD		1
#define BIEL_FHD_KD		3
#define LAIBAO_HD_KD	5
#define LAIBAO_FHD_KD	7
...
{
    /*
     * for single build image (Z301M & Z301MF)
     *
     * FT5726_TP_ID 
     *
     * 0 : BIEL_HD_BOE
    ...
    if(FT5726_TP_ID == 0 || FT5726_TP_ID == 4 || FT5726_TP_ID == 1 || FT5726_TP_ID == 5) // HD
    {
		pdata->panel_minx = 0;
		pdata->panel_miny = 0;
		pdata->panel_maxx = 1280;
		pdata->panel_maxy = 800;
		pdata->x_min = 0;
		pdata->y_min = 0;
		pdata->x_max = 1280;
		pdata->y_max = 800;
		...
		pdata->panel_minx = 0;
		pdata->panel_miny = 0;
		pdata->panel_maxx = 1920;
		pdata->panel_maxy = 1200;
		pdata->x_min = 0;
		pdata->y_min = 0;
		pdata->x_max = 1920;
		pdata->y_max = 1200;
	...
	LDBG("FT5726_TP_ID [%d] \n", FT5726_TP_ID);
	LDBG("panel_min_x [%d] panel_min_y [%d] panel_max_x [%d] panel_max_y [%d] \n",
            pdata->panel_minx, pdata->panel_miny, pdata->panel_maxx, pdata->panel_maxy);
	LDBG("touch_min_x [%d] touch_min_y [%d] touch_max_x [%d] touch_max_y [%d] \n",
...
static ssize_t touch_switch_name(struct switch_dev *sdev, char *buf)
{
	switch(FT5726_TP_ID) 
	{
		case BIEL_HD_BOE:
		case BIEL_HD_KD:
			return sprintf(buf, "BIEL_HD_V%X.%d.%d\n", private_ts->fw_ver[0], private_ts->fw_ver[1], private_ts->fw_ver[2]);
			break;
		case BIEL_FHD_BOE:
		case BIEL_FHD_KD:
			return sprintf(buf, "BIEL_FHD_V%X.%d.%d\n", private_ts->fw_ver[0], private_ts->fw_ver[1], private_ts->fw_ver[2]);
			break;
		case LAIBAO_HD_BOE:
		case LAIBAO_HD_KD:
			return sprintf(buf, "LAIBAO_HD_V%X.%d.%d\n", private_ts->fw_ver[0], private_ts->fw_ver[1], private_ts->fw_ver[2]);
			break;
		case LAIBAO_FHD_BOE:
		case LAIBAO_FHD_KD:
			return sprintf(buf, "LAIBAO_FHD_V%X.%d.%d\n", private_ts->fw_ver[0], private_ts->fw_ver[1], private_ts->fw_ver[2]);
			break;
		default:
...
#endif
	/*    
    #define FOCAL_BIEL_HD_FW_PATH "BIEL_HD_FW.bin"
    #define FOCAL_BIEL_FHD_FW_PATH "BIEL_FHD_FW.bin"
    #define FOCAL_LAIBAO_HD_FW_PATH "LAIBAO_HD_FW.bin"
    #define FOCAL_LAIBAO_FHD_FW_PATH "LAIBAO_FHD_FW.bin"
    */
	switch(FT5726_TP_ID) 
	{
		case BIEL_HD_BOE:
		case BIEL_HD_KD:
			fts_ctpm_fw_upgrade_with_app_file(fts_wq_data->client, FOCAL_BIEL_HD_FW_PATH);
			break;
		case BIEL_FHD_BOE:
			fts_ctpm_fw_upgrade_with_app_file(fts_wq_data->client, FOCAL_BIEL_FHD_FW_PATH);
			break;
		case BIEL_FHD_KD:
			fts_ctpm_fw_upgrade_with_app_file(fts_wq_data->client, FOCAL_BIEL_FHD_KD_FW_PATH);
			break;
		case LAIBAO_HD_BOE:
		case LAIBAO_HD_KD:
			fts_ctpm_fw_upgrade_with_app_file(fts_wq_data->client, FOCAL_LAIBAO_HD_FW_PATH);
			break;
		case LAIBAO_FHD_BOE:
		case LAIBAO_FHD_KD:
			fts_ctpm_fw_upgrade_with_app_file(fts_wq_data->client, FOCAL_LAIBAO_FHD_FW_PATH);
			break;
		default:
...
static void fts_read_lcm_tp_id(struct i2c_client *client)
{
	/* Check LCM / TP ID 
     LCM ID
     GPIO_201   LCM_ID0 (Pin20)
     GPIO_200   LCM_ID1 (Pin21)
     BOE (HD)   0   0
     KD (HD)    0   1
     BOE (FHD)  1   0
     KD (FHD)   1   1
     ...
     TP ID
     GPIO_203   TP_ID0 (Pin10)      
     GPIO_202   TP_ID1 (Pin11)
     LAIBAO (WHITE)    0   1
     LAIBAO (BLACK)    1   1
     BIEL (WHITE)      1   0
     BIEL (BLACK)      0   0
    */
    ...
    lcm_id_0_gpio = of_get_named_gpio(client->dev.of_node, "focaltech,lcmid0-gpio", 0);
	err = gpio_request(lcm_id_0_gpio, "lcm_id_0");
	if(err < 0) LDBG("Failed to request GPIO %d (lcm_id_0_gpio) error=%d\n", lcm_id_0_gpio, err);
    lcm_id_0 = gpio_get_value(lcm_id_0_gpio);

    lcm_id_1_gpio = of_get_named_gpio(client->dev.of_node, "focaltech,lcmid1-gpio", 0);
	err = gpio_request(lcm_id_1_gpio, "lcm_id_1");
	if(err < 0) LDBG("Failed to request GPIO %d (lcm_id_1_gpio) error=%d\n", lcm_id_1_gpio, err);
    lcm_id_1 = gpio_get_value(lcm_id_1_gpio);

    tp_id_0_gpio = of_get_named_gpio(client->dev.of_node, "focaltech,tpid0-gpio", 0);
	err = gpio_request(tp_id_0_gpio, "tp_id_0");
	if(err < 0) LDBG("Failed to request GPIO %d (tp_id_0_gpio) error=%d\n", tp_id_0_gpio, err);
    tp_id_0 = gpio_get_value(tp_id_0_gpio);
    
    tp_id_1_gpio = of_get_named_gpio(client->dev.of_node, "focaltech,tpid1-gpio", 0);
	err = gpio_request(tp_id_1_gpio, "tp_id_1");
	if(err < 0) LDBG("Failed to request GPIO %d (tp_id_1_gpio) error=%d\n", tp_id_1_gpio, err);
    tp_id_1 = gpio_get_value(tp_id_1_gpio);

	LDBG("LCM ID GPIO [%d %d] = [%X %X] \n", lcm_id_0_gpio, lcm_id_1_gpio, lcm_id_0, lcm_id_1);
	LDBG("TP ID GPIO [%d %d] = [%X %X] \n", tp_id_0_gpio, tp_id_1_gpio, tp_id_0, tp_id_1);
    ...
    /*
        ...
        Only Use tp_id_1 & lcm_id_0 to update firmware (BIEL/LAIBAO & HD/FHD)
     */
//	FT5726_TP_ID = (tp_id_1 == 0) ? ((lcm_id_0 == 0) ? 0 : 2) : ((lcm_id_0 == 0) ? 4 : 6);
	if(lcm_id_1==1){
		if(tp_id_1==0){
			if(lcm_id_0==0){
				FT5726_TP_ID=1;	//(0,0,1)
				LDBG("alp.D : TP_ID : 1\n");	
			}
			else{
				FT5726_TP_ID=3;	 //(0,1,1)
				LDBG("alp.D : TP_ID : 3\n");	
			}
		}else{
			if(lcm_id_0==0){
				FT5726_TP_ID=5;	//(1,0,1)
				LDBG("alp.D : TP_ID : 5\n");	
			}
			else{
				FT5726_TP_ID=7;	 //(1,1,1)
				LDBG("alp.D : TP_ID : 7\n");	
			}		
		}
	}else{
		if(tp_id_1==0){
			if(lcm_id_0==0){
				FT5726_TP_ID=0;	//(0,0,0)
				LDBG("alp.D : TP_ID : 0\n");	
			}
			else{
				FT5726_TP_ID=2;	 //(0,1,0)
				LDBG("alp.D : TP_ID : 2\n");	
			}
		}else{
			if(lcm_id_0==0){
				FT5726_TP_ID=4;	//(1,0,0)
				LDBG("alp.D : TP_ID : 4\n");	
			}
			else{
				FT5726_TP_ID=6;	 //(1,1,0)
				LDBG("alp.D : TP_ID : 6\n");	
			}		
		}
	}
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    LDBG("Driver Build Version : 2017/09/27\n");
...
*/
	...
	*/
	...
	LDBG("fw_vendor_id = %d \n", data->fw_vendor_id);
	LDBG("FW version check and auto-update (data->fw_ver[0] = %2X) ... \n", data->fw_ver[0]);
	if(build_version == 1){
		...
		switch(FT5726_TP_ID) 
		{
			case BIEL_HD_BOE:
			case BIEL_HD_KD:
				if(data->fw_ver[0] < 0x9C) {
					queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
					LDBG("[BIEL_HD] force update to version: [0x%2X] -> [0x9C]\n", data->fw_ver[0]);
				} else {
					LDBG("[BIEL_HD][V%2X] check version pass, skip force FW update\n", data->fw_ver[0]);
				}
				break;
			case BIEL_FHD_BOE:
				if(data->fw_ver[0] < 0xD6) {
					queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
					LDBG("[BIEL_FHD_BOE] force update to version: [0x%2X] -> [0xD6]\n", data->fw_ver[0]);
				} else {
					LDBG("[BIEL_FHD_BOE][V%2X] check version pass, skip force FW update\n", data->fw_ver[0]);
				}
				break;
			case BIEL_FHD_KD:
				if(data->fw_ver[0] < 0xD8) {
					queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
					LDBG("[BIEL_FHD_KD] force update to version: [0x%2X] -> [0xD8]\n", data->fw_ver[0]);
				} else {
					LDBG("[BIEL_FHD_KD][V%2X] check version pass, skip force FW update\n", data->fw_ver[0]);
				}
				break;
			case LAIBAO_HD_BOE:
			case LAIBAO_HD_KD:
				if(data->fw_ver[0] < 0x9D) {
					queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
					LDBG("[LAIBAO_HD] force update to version: [0x%2X] -> [0x9D]\n", data->fw_ver[0]);
				} else {
					LDBG("[LAIBAO_HD][V%2X] check version pass, skip force FW update\n", data->fw_ver[0]);
				}
				break;
			case LAIBAO_FHD_BOE:
				if(data->fw_ver[0] < 0xD7) {
					queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
					LDBG("[LAIBAO_FHD_BOE] force update to version: [0x%2X] -> [0xD7]\n", data->fw_ver[0]);
				} else {
					LDBG("[LAIBAO_FHD_BOE][V%2X] check version pass, skip force FW update\n", data->fw_ver[0]);
				}
				break;
			case LAIBAO_FHD_KD:
				if(data->fw_ver[0] < 0xD7) {
					queue_delayed_work(data->ts_update_workqueue, &data->touch_update_work, 15 * HZ);
					LDBG("[LAIBAO_FHD_KD] force update to version: [0x%2X] -> [0xD7]\n", data->fw_ver[0]);
				} else {
					LDBG("[LAIBAO_FHD_KD][V%2X] check version pass, skip force FW update\n", data->fw_ver[0]);
				}
				break;
			default:
				LDBG("Unknown FT5726_TP_ID [%d]\n", FT5726_TP_ID);
