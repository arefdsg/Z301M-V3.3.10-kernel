 ...
 *  Global Variable
 */
extern int chg_timeout_limit_stopFG;
bool g_ChkBatTemp_flag = 0;
int pre_chg_type = 0;//for decrease logs
struct battery_gpio{
unsigned int    usb_alert;
};
struct battery_gpio *battery_gpio;
int g_bootup_skip_count = 1;//for check soc_update count at start
int g_boot_up_first = 1;//for check first boot to change UI_SOC
bool b_COS_CO = false;
bool is_full_charging = false;
int g_full_cnt = 0;
unsigned int charger_temp = 0;
...
__attribute__ ((weak)) void ssusb_mode_switch_typec(int to_host) {}
...
{
	if (g_ChkBatTemp_flag == 1)
		return 25;
	else
		return BMT_status.temperature_now;
}
...
__attribute__ ((weak)) bool mt_usb_pd_support(void) { return KAL_FALSE; }
__attribute__ ((weak)) bool mt_is_power_sink(void) { return KAL_TRUE; }
...
static void ADB_CMD_check_battery_id(int cmd){
		if(cmd == 0){
				if(g_battery_id == -1){
						battery_log(BAT_LOG_CRTI, "[%s] Fail!!\n",__func__);
				}else{
						battery_log(BAT_LOG_CRTI, "[%s] Pass!!\n",__func__);
				}
		}else if(cmd == 1){
				if(g_battery_id == 0){
						battery_log(BAT_LOG_CRTI, "[%s] LG!!\n",__func__);
				}else if(g_battery_id == 1){
						battery_log(BAT_LOG_CRTI, "[%s] SDI!!\n",__func__);
				}else if(g_battery_id == -1){
						battery_log(BAT_LOG_CRTI, "[%s] Wrong ID!!\n",__func__);
				}
		}else{
				battery_log(BAT_LOG_CRTI, "[%s] Wrong Command!!\n",__func__);
		}
		return;
}
				...
				enum power_supply_property psp,const union power_supply_propval *val)
{
		...
		switch (psp) {
				case POWER_SUPPLY_PROP_battery_id:
						ADB_CMD_check_battery_id(val->intval);
						break;
				default:
						ret = -EINVAL;
		}
...
//		val->intval = data->BAT_batt_vol;
	...
	case POWER_SUPPLY_PROP_current_now:
		battery_charging_control(CHARGING_CMD_GET_bq25896_ICharging,&ret);
		val->intval = ret;
		break;
		/* Dual battery */
	...
	case POWER_SUPPLY_PROP_adjust_power:
		...
		break;
	case POWER_SUPPLY_PROP_battery_id:
		val->intval = g_battery_id;
		break;
	case POWER_SUPPLY_PROP_usb_alert:
		val->intval = gpio_get_value(battery_gpio->usb_alert);
		battery_log(BAT_LOG_CRTI, "[%s]usb_alert = %d\n",__func__,val->intval);
		break;
...
#if !defined(CONFIG_POWER_EXT)
static ssize_t show_ChkBatTemp(struct device *dev, struct device_attribute *attr,
                                        char *buf)
{
	battery_log(BAT_LOG_CRTI, "[%s] ChkBatTemp = %d\n",__func__, g_ChkBatTemp_flag);
        return sprintf(buf, "%d\n", g_ChkBatTemp_flag);
}

static ssize_t store_ChkBatTemp(struct device *dev, struct device_attribute *attr,
                                         const char *buf, size_t size)
{
	int request;
        ...
        if (!request){
                g_ChkBatTemp_flag = false;
                battery_log(BAT_LOG_CRTI, "[%s] ChkBatTemp = %d, normal work\n",__func__,g_ChkBatTemp_flag);
        }else{
                g_ChkBatTemp_flag = true;
                battery_log(BAT_LOG_CRTI, "[%s] ChkBatTemp = %d, ignore check battery temp notification\n",__func__,g_ChkBatTemp_flag);
        }
        return size;
}

static DEVICE_ATTR(ChkBatTemp, 0664, show_ChkBatTemp, store_ChkBatTemp);
...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
	...
	if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
		...
		/* reduce after xxs */
		if (chr_wake_up_bat == KAL_FALSE && bq25896_ICharging == 0 && chr_wake_up_for_meter_check == 0) {
			if (timer_counter ==
			...
			if(chr_wake_up_for_meter_check == 1){
				battery_log(BAT_LOG_FULL,"[%s] Cable Out Interrupt!!! Do not update UI_SOC.\n",__func__);
			}
		}
#else
		BMT_status.UI_SOC--;
#endif
		battery_log(BAT_LOG_CRTI, "[%s] UI_SOC=%d, SOC=%d, counter = %d\n",__func__,
		...
		if(g_boot_up_first){
			...
			if ((BMT_status.UI_SOC < BMT_status.SOC) && (chr_wake_up_for_meter_check == 0)){
...
{
	static unsigned int bq25896_PGS;
	static int VoltDelta;
	static unsigned int bq25896_ICharging;
	...
	if(g_ChkBatTemp_flag == 1)
		bat_data->BAT_batt_temp = 250;
	else
		bat_data->BAT_batt_temp = BMT_status.temperature_now * 10;
	...
	**Positive oam_i_2 == discharging		  */
	battery_charging_control(CHARGING_CMD_GET_bq25896_ICharging,&bq25896_ICharging);//Charging Current
	battery_charging_control(CHARGING_CMD_GET_bq25896_PowerGoodStatus,&bq25896_PGS);//power_good_status 1:with cable    0:without cable
	battery_charging_control(CHARGING_CMD_GET_VREG,&bq25896_VREG);					//CV Point
	VBAT = battery_meter_get_battery_voltage(KAL_TRUE);
	VoltDelta = BMT_status.bat_vol_now - BMT_status.bat_vol;
	if(VBAT < bq25896_VREG && g_boot_up_first == 0){
		if(bq25896_ICharging > 0){
				if(oam_i_2 > 0){
						battery_log(BAT_LOG_CRTI, "[%s]MeterReset!!(%d) oam_i_2(%d) bq25896_ICharging(%d)\n",__func__, BMT_status.UI_SOC, oam_i_2, bq25896_ICharging);
						resetBatteryMeter = KAL_TRUE;
				}
		}else{
				if(((oam_i_2 > 0) && (VoltDelta > 0)) || ((oam_i_2 < 0) && (VoltDelta < 0))){
						battery_log(BAT_LOG_CRTI, "[%s]MeterReset!!(%d) oam_i_2(%d) VoltDelta(%d)\n",__func__, BMT_status.UI_SOC, oam_i_2, VoltDelta);
						resetBatteryMeter = KAL_TRUE;
				}
		}
	}

	if((bq25896_PGS == 1) && (VoltDelta < 0)){
			battery_log(BAT_LOG_CRTI, "[%s] Charging Current Is Not Enough!!!\n",__func__);
	}

	if(chg_timeout_limit_stopFG && (BMT_status.UI_SOC == 60)){
			battery_log(BAT_LOG_CRTI, "[%s] chg_timeout_limit_stopFG is true and UI_SOC = 60, Keep UI_SOC!!!\n",__func__);
			resetBatteryMeter = KAL_TRUE;
	}
	...
	battery_log(BAT_LOG_CRTI, "VBAT(%d) VREG(%d) oam_i_2(%d) bq25896_ICharging(%d) PowerGoodStatus(%d) bat_vol_now(%d) bat_vol(%d) UI_SOC(%d) SOC(%d) resetBatteryMeter(%d)\n", VBAT, bq25896_VREG, oam_i_2, bq25896_ICharging, bq25896_PGS, BMT_status.bat_vol_now, BMT_status.bat_vol, BMT_status.UI_SOC, BMT_status.SOC, resetBatteryMeter);
					   ...
					   unsigned int data, signed int *sum)
{
	...
	if ((batteryBufferFirst == KAL_TRUE) || (chr_wake_up_for_meter_check == KAL_TRUE)) {
...
//	unsigned int bat_vol, charger_vol, Vsense, ZCV;
#if defined(CONFIG_Z380M)
	unsigned int bat_vol, charger_vol, Vsense, ZCV, pack_vol;
#else
	unsigned int bat_vol, charger_vol, Vsense, ZCV;
#endif
	...
	if (bat_meter_timeout == KAL_TRUE || bat_spm_timeout == TRUE || fg_wake_up_bat == KAL_TRUE || _g_bat_sleep_short_total_time >= 60) {
		...
		if(BMT_status.charger_exist == KAL_TRUE){//Charging
				if(is_full_charging == KAL_TRUE){
						//Charger IC consider it as charging full
						//bat_full until Cable Out
						BMT_status.bat_charging_state = CHR_BATFULL;
						BMT_status.bat_full = KAL_TRUE;
						g_charging_full_reset_bat_meter = KAL_TRUE;
						g_full_cnt = 0;
				}else if((BMT_status.UI_SOC == 99) && (charger_temp > 51) && (charger_temp < 112) && (bat_vol > 4300)){
						//UI_SOC = 99%
						//bq25896 temp should in the range 0-45degree. 0degree 1110000 => 112 ; 45degree 0110011 => 51
						//VBUS exist
						//VBAT > 4.3V
						if(g_full_cnt < 10){
								g_full_cnt++;
		...
		battery_log(BAT_LOG_CRTI,"[%s] is_full_charging(%d), UI_SOC(%d), SOC(%d), charger_temp(%d), bat_vol(%d), g_full_cnt(%d)\n", __func__, is_full_charging, BMT_status.UI_SOC, SOC, charger_temp, bat_vol,g_full_cnt);
	} else {
		if (previous_SOC == -1)
			SOC = battery_meter_get_battery_percentage();
		else
...
/*	battery_log(BAT_LOG_CRTI,
	...
	/*There are three parameters set as average, so keep the real time value to xxx_now*/
	BMT_status.bat_vol_now = bat_vol;
	BMT_status.ICharging_now = ICharging;
...
#else
	battery_log(BAT_LOG_CRTI,
		    "AvgVbat=(%d),bat_vol=(%d),AvgI=(%d),I=(%d),VChr=(%d),AvgT=(%d),T=(%d),pre_SOC=(%d),SOC=(%d),UI-SOC=(%d),ZCV=(%d),is_full_charging=(%d)\n",
		    BMT_status.bat_vol, bat_vol, BMT_status.ICharging, ICharging,
		    BMT_status.charger_vol, BMT_status.temperature, temperature,
		    previous_SOC, BMT_status.SOC, BMT_status.UI_SOC, BMT_status.ZCV,is_full_charging);
#endif
}
...
void mt_battery_notify_check(void)
{
	...
	if (g_BN_TestMode == 0x0000) {	/* for normal case */
		...
		if(g_ChkBatTemp_flag == 0){
			...
			battery_log(BAT_LOG_CRTI, "[%s] Not Do mt_battery_notify_VBatTemp_check!!!!!\n",__func__);
		}
...
//		if (BMT_status.temperature >= 60) {
		if (BMT_status.temperature_now >= 60) {
#if defined(CONFIG_POWER_EXT)
			battery_log(BAT_LOG_CRTI,
...
#if defined(CONFIG_USB_MTK_CHARGER_DETECT)
	if (mt_get_usb11_port_status() == true)	{
		battery_log(BAT_LOG_FULL, "========use_usb_detect===========\n");
		CHR_Type_num = usb_charger_type_detect();
		BMT_status.charger_type = CHR_Type_num;
		battery_log(BAT_LOG_CRTI, "[%s] charger_type(%d)\n",__func__,CHR_Type_num);
	} else
#endif
#endif
	{
		battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE, &CHR_Type_num);
		BMT_status.charger_type = CHR_Type_num;
		battery_log(BAT_LOG_CRTI, "[%s] Read throught bq25896-interface, charger_type(%d)\n",__func__,CHR_Type_num);
#ifdef ENG_BUILD
...
#endif
	if (upmu_is_chr_det() == KAL_TRUE) {
		battery_log(BAT_LOG_FULL, "[%s] VBUS exist\n",__func__);
		...
		if ((BMT_status.charger_type == STANDARD_HOST) || (BMT_status.charger_type == CHARGING_HOST)) {
			mt_usb_connect();//USB connect
		}
#else
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
		if (BMT_status.charger_type == CHARGER_UNKNOWN) {
			battery_log(BAT_LOG_CRTI, "[%s] charger_type =Unknown\n",__func__);
#else
		if ((BMT_status.charger_type == CHARGER_UNKNOWN) &&
		    (DISO_data.diso_state.cur_vusb_state == DISO_ONLINE)) {
#endif
			mt_charger_type_detection();//Type Detection
...
//		battery_charging_control(CHARGING_CMD_SET_CHRIND_CK_PDN, &pwr);
#endif
		if(pre_chg_type != BMT_status.charger_type){
				battery_log(BAT_LOG_CRTI, "[BAT_thread]Cable Exist, CHR_Type_num=%d\r\n", BMT_status.charger_type);
		}
	} else {
		...
		if(pre_chg_type != BMT_status.charger_type){
				battery_log(BAT_LOG_CRTI, "[BAT_thread]Cable Not exist \r\n");
		}
		mt_usb_disconnect();
...
//		battery_charging_control(CHARGING_CMD_SET_CHRIND_CK_PDN, &pwr);
	...
	pre_chg_type = BMT_status.charger_type;
}
...
void wake_up_bat_for_int(void){
		...
		if (g_battery_soc_ready == KAL_FALSE) {
				battery_log(BAT_LOG_FULL, "[%s] g_battery_soc_ready = 0\n", __func__);
				if (BMT_status.nPercent_ZCV == 0)
						battery_meter_initial();

				battery_log(BAT_LOG_FULL, "[%s] battery_meter_get_battery_percentage\n", __func__);
				BMT_status.SOC = battery_meter_get_battery_percentage();
		}

		if (BMT_status.bat_vol > 0) {
				battery_log(BAT_LOG_FULL, "[%s] mt_battery_update_status\n", __func__);
				mt_battery_update_status();
				battery_log(BAT_LOG_FULL, "[%s] battery_meter_get_battery_percentage\n", __func__);
				BMT_status.SOC = battery_meter_get_battery_percentage();
				skip_battery_update = KAL_TRUE;
		}
		battery_log(BAT_LOG_FULL, "[%s] wake_up_bat\n", __func__);
		wake_up_bat();
}

void touch_for_int(void){
#if defined(CONFIG_Z380M)
...
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
		if (upmu_is_chr_det() == KAL_TRUE && chr_wake_up_for_meter_charging == 1) {
#else
...
//			}
			...
			cable_in_uevent = 1;
            battery_log(BAT_LOG_CRTI, "[%s] Plug In battery update\n", __func__);
			wake_up_bat_for_int();
		} else if(upmu_is_chr_det() == KAL_FALSE && chr_wake_up_for_meter_discharging == 1) {
...
//			battery_charging_control(CHARGING_CMD_ENABLE, &BMT_status.charger_exist);
			...
			if (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
				...
				if (mt_usb_pd_support())
					msleep(1000);
				if(b_COS_CO == false){
					battery_log(BAT_LOG_CRTI,"[%s] Unplug Charger/USB In COS Mode!  Shutdown OS!\r\n",__func__);
					battery_charging_control(CHARGING_CMD_SET_POWER_OFF, NULL);
					/* mt_power_off(); */
				}else{
...
#endif
            battery_log(BAT_LOG_CRTI, "[%s] Plug Out battery update\n", __func__);
			wake_up_bat_for_int();
#if 0
#if defined(CONFIG_POWER_EXT)
			mt_usb_disconnect();
			...
			if(upmu_is_chr_det() == KAL_FALSE){
				battery_log(BAT_LOG_CRTI, "[%s] VBUS is already Off, trigger too many times!!!\n", __func__);
			}else{
				battery_log(BAT_LOG_CRTI, "[%s] VBUS is already On, trigger too many times!!!\n", __func__);
			}
		}
		/*Finish Wake up BAT_thread, call touch function*/
		touch_for_int();
#if defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
...
void BAT_thread(void)
{
	...
	if (chargin_hw_init_done){
			...
			battery_log(BAT_LOG_FULL, "[%s]Clear bq25896 watchdog\n",__func__);
	}
	/******************************/
	...
	battery_log(BAT_LOG_FULL, "[%s]after mt_battery_GetBatteryData\n",__func__);
	if (BMT_status.charger_exist == KAL_TRUE){
		check_battery_exist();
		battery_log(BAT_LOG_FULL, "[%s]after check_battery_exist\n",__func__);
	}
	mt_battery_thermal_check();
	battery_log(BAT_LOG_FULL, "[%s]after mt_battery_thermal_check\n",__func__);
#ifndef ENG_BUILD
	mt_battery_notify_check();
	battery_log(BAT_LOG_CRTI, "[%s]after mt_battery_notify_check\n",__func__);
#endif
	if (BMT_status.charger_exist == KAL_TRUE) {
		mt_battery_CheckBatteryStatus();
		battery_log(BAT_LOG_FULL,"[%s]Before mt_battery_charging_algorithm\n",__func__);
		mt_battery_charging_algorithm();
		battery_log(BAT_LOG_FULL,"[%s]After mt_battery_charging_algorithm\n",__func__);
	}

	mt_battery_update_status();
	battery_log(BAT_LOG_FULL,"[%s]After update status\n",__func__);
	mt_kpoc_power_off_check();
	battery_log(BAT_LOG_FULL,"[%s]After COS Check\n",__func__);
}
...
battery_log(BAT_LOG_CRTI,"[%s] chargin_hw_init_done(%d)battery_suspended(%d)chargin_hw_init_done(%d)fg_wake_up_bat(%d)\n",__func__,chargin_hw_init_done,battery_suspended,chargin_hw_init_done,fg_wake_up_bat);
		if (((chargin_hw_init_done == KAL_TRUE) && (battery_suspended == KAL_FALSE))
		    || ((chargin_hw_init_done == KAL_TRUE) && ((fg_wake_up_bat == KAL_TRUE) || (chr_wake_up_bat == KAL_TRUE)))){
			BAT_thread();
		...
		get_monotonic_boottime(&last_bat_time);
		battery_log(BAT_LOG_CRTI,"[%s] after wait_event(%ld)\n",__func__, last_bat_time.tv_sec);
...
*/
	...
	/* For EM */
	{
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_ChkBatTemp);
	...
	if (upmu_is_chr_det() == KAL_TRUE){
			battery_log(BAT_LOG_CRTI, "[%s] charger exist! Init chr_wake_up_for_meter!! chg(%d)dis(%d)check(%d)\n",__func__,chr_wake_up_for_meter_charging,chr_wake_up_for_meter_discharging,chr_wake_up_for_meter_check);
			BMT_status.charger_exist = KAL_TRUE;
			chr_wake_up_for_meter_charging = 0;
			chr_wake_up_for_meter_discharging = 1;
			chr_wake_up_for_meter_check = 1;
	}else{
			battery_log(BAT_LOG_CRTI, "[%s] charger NOT exist! Init chr_wake_up_for_meter!! chg(%d)dis(%d)check(%d)\n",__func__,chr_wake_up_for_meter_charging,chr_wake_up_for_meter_discharging,chr_wake_up_for_meter_check);
			BMT_status.charger_exist = KAL_FALSE;
			BMT_status.charger_type = CHARGER_UNKNOWN;
            chr_wake_up_for_meter_charging = 1;
            chr_wake_up_for_meter_discharging = 0;
	...
	touch_for_int();
	battery_log(BAT_LOG_CRTI, "[battery_probe] touch_for_int Done\n");
...
//	hvtime = ktime_set(0, BAT_MS_TO_NS(2000));
	...
	_g_rest_bat_time = BAT_TASK_PERIOD - (bat_time_after_sleep.tv_sec - last_bat_time.tv_sec);
	battery_log(BAT_LOG_CRTI, "reset bat_time(%d),BAT_TASK_PERIOD(%ld)after_sleep(%ld),last_soc(%ld)\n", _g_rest_bat_time, BAT_TASK_PERIOD, bat_time_after_sleep.tv_sec, last_bat_time.tv_sec);
	if (is_pcm_timer_trigger == KAL_TRUE || bat_spm_timeout || _g_bat_sleep_short_total_time >= 60 || _g_rest_bat_time <= 0) {
		mutex_lock(&bat_mutex);
		BAT_thread();
		mutex_unlock(&bat_mutex);
	} else {
...
static int battery_dts_probe(struct platform_device *dev)
{
	...
	if (battery_device.dev.of_node) {
			battery_gpio = devm_kzalloc(&dev->dev, sizeof(*battery_gpio), GFP_KERNEL);
			battery_log(BAT_LOG_CRTI, "try to get usb_alert_gpio\n");
			battery_gpio->usb_alert = of_get_named_gpio(battery_device.dev.of_node, "usb_alert_gpio", 0);
			battery_log(BAT_LOG_CRTI, "usb_alert_gpio = %d\n", battery_gpio->usb_alert);
			ret = gpio_request(battery_gpio->usb_alert, "usb_alert");
			if(ret<0){
					gpio_free(battery_gpio->usb_alert);
					battery_log(BAT_LOG_CRTI,"%s: gpio_request failed for gpio usb_alert = %d\n", __func__, battery_gpio->usb_alert);
			}else{
					battery_log(BAT_LOG_CRTI,"%s: usb_alert_gpio request success and set input(%d)\n", __func__, battery_gpio->usb_alert);
					gpio_direction_input(battery_gpio->usb_alert);
			}
	}
