...
void BAT_thread(void)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE){
		check_battery_exist();
		battery_log(BAT_LOG_CRTI, "[%s]after check_battery_exist\n",__func__);
	}
	mt_battery_thermal_check();
	battery_log(BAT_LOG_CRTI, "[%s]after mt_battery_thermal_check\n",__func__);
	mt_battery_notify_check();
	battery_log(BAT_LOG_CRTI, "[%s]after mt_battery_notify_check\n",__func__);
	if (BMT_status.charger_exist == KAL_TRUE) {
		mt_battery_CheckBatteryStatus();
		battery_log(BAT_LOG_CRTI,"[%s]Before mt_battery_charging_algorithm\n",__func__);
		mt_battery_charging_algorithm();
		battery_log(BAT_LOG_CRTI,"[%s]After mt_battery_charging_algorithm\n",__func__);
	}
