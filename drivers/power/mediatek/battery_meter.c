...
void fgauge_get_profile_id(void)
{
	...
	/*if ((150000 < id_volt) && (id_volt < 178000)) {
		g_fg_battery_id = 0;
		g_battery_id = g_fg_battery_id;
	}else*/if((849000 < id_volt) && (id_volt < 953000)){//only one battery table =>100kohm, LG
		g_fg_battery_id = 1;
		g_battery_id = g_fg_battery_id;
	}
#elif defined(CONFIG_Z380M)
	if ((568000 < id_volt) && (id_volt < 650000)) {
		g_fg_battery_id = 0;
		g_battery_id = g_fg_battery_id;
	}else if((849000 < id_volt) && (id_volt < 953000)){
		g_fg_battery_id = 1;
		g_battery_id = g_fg_battery_id;
	}
	...
	if ((150000 < id_volt) && (id_volt < 178000)) {
		...
		g_fg_battery_id = 0;
		g_battery_id = -1;
		battery_log(BAT_LOG_CRTI, "[fgauge_get_profile_id]Battery id Out of Range(%d)\n",g_battery_id);
	}
	battery_log(BAT_LOG_CRTI, "[fgauge_get_profile_id]Battery id (%d),(%d)\n", g_fg_battery_id,g_battery_id);
...
//	getrawmonotonic(&now_time);
	...
	battery_log(BAT_LOG_CRTI,"chr_wake_up_for_meter_check = %d, chr_wake_up_for_meter_charging = %d, chr_wake_up_for_meter_discharging = %d, g_otg_enable = %d\n",chr_wake_up_for_meter_check,chr_wake_up_for_meter_charging,chr_wake_up_for_meter_discharging,g_otg_enable);
	if(bat_is_charger_exist() == KAL_TRUE && (chr_wake_up_for_meter_charging == KAL_FALSE) && (chr_wake_up_for_meter_discharging == KAL_TRUE)){
		...
		battery_log(BAT_LOG_CRTI,"[%s]OTG!! Discharging Wake time\n",__func__);
		getrawmonotonic(&now_time);
	}else{
...
signed int battery_meter_initial(void)
{
	...
	battery_log(BAT_LOG_CRTI,"[%s]meter_initilized(%d)",__func__,meter_initilized);
	mutex_lock(&FGADC_mutex);
	if (meter_initilized == KAL_FALSE) {
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
		battery_log(BAT_LOG_CRTI,"[%s]before fgauge_get_profile_id, meter_initilized(%d)",__func__,meter_initilized);
		fgauge_get_profile_id();
		battery_log(BAT_LOG_CRTI,"[%s]after fgauge_get_profile_id, meter_initilized(%d)",__func__,meter_initilized);
#endif
...
//	getrawmonotonic(&now_time);
	if(bat_is_charger_exist() == KAL_FALSE){
		getrawmonotonic(&now_time);
		battery_log(BAT_LOG_CRTI,"[%s] Discharging, Wake!!\n",__func__);
	}else{
		get_monotonic_boottime(&now_time);
		battery_log(BAT_LOG_CRTI,"[%s] Charging, Wake + Sleep!!\n",__func__);
	}
