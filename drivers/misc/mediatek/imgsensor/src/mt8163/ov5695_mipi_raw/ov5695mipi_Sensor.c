...
static int camera_proc_otp_af_read(struct seq_file *buf, void *v)
{
	seq_printf(buf, "%s\n", camera_module_otp_af);
	return 0;
}

static int camera_proc_otp_af_open(struct inode *inode, struct  file *file) {
	return single_open(file, camera_proc_otp_af_read, NULL);
}

static const struct file_operations camera_otp_af_fops = {
		.owner = THIS_MODULE,
		.open = camera_proc_otp_af_open,
		.read = seq_read,
};
...
static void create_camera_proc_file(void)
{
	...
	camera_proc_otp_af_file = proc_create(CAMERA_PROC_OTP_AF_FILE, 0666, NULL, &camera_otp_af_fops);
	if(camera_proc_otp_af_file){
		printk("proc otp_af file create sucessed!\n");
	}
	else{
		printk("proc otp_af file create failed!\n");
	}
}
//ASUS_BSP---, ATD
...
//ASUS_BSP+++, ATD
static int ov5695_otp_read(void)
{
	int i;
	int start_addr = 0;
	int otp_flag;
	...
	if((otp_flag & 0xc0) == 0x40) {
		start_addr = 0x700d; //Group 0
	} else if((otp_flag & 0x30) == 0x10) {
		start_addr = 0x7014; //Group 1
	} else {
		LOG_INF("OTP is invalid\n");
		start_addr = 0;
	}
	...
	if(start_addr != 0) {
		for (i = 0; i < 4; i++) {
			...
			//ov5695_otp_data[i] = (u8)read_value[i];
			//printk("%s Check Data Package %d, 0x%X\n", __func__, package, read_value[i]);
		}
		printk("%s AF INF HSB 0x%X, LSB 0x%X\n", __func__, read_value[0], read_value[1]);
		af_inf_temp = (read_value[0] << 8) | read_value[1];
		printk("%s AF af_inf_temp 0x%X\n", __func__, af_inf_temp);
		af_inf_temp = af_inf_temp - 50;
		printk("%s Modified AF af_inf_temp 0x%X\n", __func__, af_inf_temp);
		read_value[0] = af_inf_temp >> 8;
		read_value[1] = af_inf_temp & 0xFF;
		printk("%s Modified AF INF HSB 0x%X, LSB 0x%X\n", __func__, read_value[0], read_value[1]);

		for (i = 0; i < 3; i++) {
			read_value[4 + i] = read_cmos_sensor(0x7011 + i); //PN
		}
		for (i = 0; i < 3; i++) {
			read_value[7 + i] = read_cmos_sensor(0x7018 + i); //DC
		}

		snprintf(camera_module_otp, sizeof(camera_module_otp), "0x%X 0x%X 0 0 0x%X 0x%X 0 0 0 0 0x%X 0x%X 0x%X 0 0 0 0 0 0x%X 0x%X 0x%X 0 0 0\n"
				, read_value[0], read_value[1], read_value[2], read_value[3], read_value[7]
				, read_value[8], read_value[9], read_value[4], read_value[5], read_value[6]);
		snprintf(camera_module_otp_uid, sizeof(camera_module_otp_uid), "000000000000000000%02X%02X%02X\n"
				, read_value[7], read_value[8],read_value[9]);
		snprintf(camera_module_otp_af, sizeof(camera_module_otp_af), "0x%X 0x%X 0x0 0x0 0x%X 0x%X\n"
				, read_value[0], read_value[1], read_value[2], read_value[3]);
	} else {
		printk("%s Dump OTP Data: ", __func__);
		for (i = 0; i < 16; i++) {
			read_value[0] = read_cmos_sensor(0x700c + i);
			printk("0x%X ", read_value[0]);
		}
		printk("\n");
	}
	...
	for (i = 0x700c; i <= 0x701b; i++) {
		write_cmos_sensor(i, 0x0);
	}

	return 0;
}

static int ov5695_dbgfs_init(void)
{
	struct dentry *debugfs_dir;

	debugfs_dir = debugfs_create_dir("camera0", NULL);
	debugfs_create_u32("camera_status", 0644, debugfs_dir, &ATD_ov5695_status);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	pcb_id = Read_PCB_ID();
	if(((proj_id == 5) || (proj_id == 3)) && (((pcb_id>>8) & 0x1) == 0)) { //Z301M/MF and OV5670
		LOG_INF("Z301M/MF + OV5670, skip OV5695 probe\n");
			...
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if(probed == 0) {
					...
					ATD_ov5695_status = 1;
					ov5695_dbgfs_init(); //ATD camera status
					probed = 1;
				}
				//ASUS_BSP---, ATD
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		if(probed == 0) {
			ATD_ov5695_status = 0;
			ov5695_dbgfs_init(); //ASUS_BSP+++ ATD camera status
			probed = 1;
		}
		//ASUS_BSP---, ATD
