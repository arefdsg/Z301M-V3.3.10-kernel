...
static struct workqueue_struct *vproc_wq;
struct delayed_work vproc_work;
...
static void vproc_function(struct work_struct *work)
{
	printk("[%s] disabble vproc\n",__func__);
	pmic_config_interface(0x21A, 0x0, 0x1, 0);
}

static int pmic_mt6323_probe(struct platform_device *dev)
{
	int proj_id = Read_PROJ_ID();
	int ret_val = 0;
	...
	vproc_wq = create_singlethread_workqueue("vproc_wq");
	INIT_DEFERRABLE_WORK(&vproc_work, vproc_function);
	if(proj_id==3){
		printk("[%s] into queue to vproc_function\n",__func__);
		queue_delayed_work(vproc_wq, &vproc_work, msecs_to_jiffies(2000));
	}
