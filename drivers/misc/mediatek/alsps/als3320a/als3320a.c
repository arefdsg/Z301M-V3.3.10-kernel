...
//		struct als3320a_priv *obj = i2c_get_clientdata(client);
		...
		if (enable) {			
			...
			als3320a_obj->high_thres = 0;
			als3320a_obj->low_thres = 36686;
...
static int als3320a_set_DummyTime(struct i2c_client *client, int val)
{
	int err;
	err = als3320a_write_reg(client, ALS3320A_ALS_DUMMY, 0xFF, 0x00, val);
	return err;
}
...
//	struct als3320a_priv *obj = i2c_get_clientdata(client);
	...
	als3320a_set_DummyTime(client, 0x3E);				//set dummy time = 6.2ms, it should be modified with ALS gain
...
//---------------------------  show cali data --------------------------------
static ssize_t als3320a_cali_show(struct device *dev, struct device_attribute *attr, char *buf)
...
static DEVICE_ATTR(show_cali, 0660, als3320a_cali_show, NULL);
...
//	struct als3320a_priv *obj = als3320a_obj;
	...
	return_value = als3320a_obj->als* ALS3320A_USE_RESOLUTION/100;//ap3xx6_get_als_value(ap3xx6_obj, ap3xx6_obj->als);

	// Add 10% debounce
	if ((return_value < als3320a_obj->high_thres) && (return_value > als3320a_obj->low_thres)){
		return_value = temp_als;
	}
	else{
		temp_als = return_value;
		als3320a_obj->high_thres = (return_value*110)/100;
		als3320a_obj->low_thres = (return_value*90)/100;
	}
...
//	als3320a_obj = obj;
	...
	err += sysfs_create_file(android_light_kobj, &dev_attr_show_cali.attr);
	if (err != 0) {
