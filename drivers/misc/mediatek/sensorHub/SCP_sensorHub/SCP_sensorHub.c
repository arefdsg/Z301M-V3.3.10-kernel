...
static void SCP_sensorHub_IPI_handler(int id, void *data, unsigned int len)
{
	...
	if (len > 48) {
		...
		if (atomic_read(&(obj_data->wait_rsp)) == 1 && true == wake_up_req) {
			...
			} else if (ID_SENSOR_MAX_HANDLE < userData->req.sensorType) {
				SCP_ERR("SCP_sensorHub_IPI_handler invalid sensor type %d\n",
					userData->req.sensorType);
			} else if (userData->req.action < 0) {
				SCP_ERR("SCP_sensorHub_IPI_handler invalid unknown action %d\n",
					userData->req.action);
				return;
			}else {
...
static int SCP_sensorHub_step_counter_init(void)
{
	...
	ctl.step_c_set_delay = ctl.step_d_set_delay = step_counter_set_delay;
	ctl.is_report_input_direct = false;
	ctl.is_support_batch = true;
