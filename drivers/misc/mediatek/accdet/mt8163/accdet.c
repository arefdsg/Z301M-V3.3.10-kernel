...
void set_spk_hs_gpio_control(int enable)
{
       if(PROJ_ID == 0x1 || PROJ_ID == 0x5 || PROJ_ID == 0x3){ //Z300M, Z301M headset spk switch (Z300M:1, Z301M:5, Z301MF:3)
               if (pin_headsetspk != 0){
                       pr_info("Force to set headset-speaker-switch GPIO to %d", enable);
                       gpio_set_value(pin_headsetspk, enable);
               } else {
                       pr_err("headset-speaker-switch GPIO could not be accessed");
               }
       }
}
EXPORT_SYMBOL(set_spk_hs_gpio_control);

int get_spk_hs_gpio_control(void)
{
       int ret = 0;

       if(PROJ_ID == 0x1 || PROJ_ID == 0x5 || PROJ_ID == 0x3){ //Z300M, Z301M headset spk switch (Z300M:1, Z301M:5, Z301MF:3)
               if (pin_headsetspk != 0){
                       ret = gpio_get_value(pin_headsetspk);
                       pr_debug("The value of headset-speaker-switch GPIO is %d", ret);
               } else {
                       pr_err("headset-speaker-switch GPIO could not be accessed");
               }
       }
...
EXPORT_SYMBOL(get_spk_hs_gpio_control);
...
static void accdet_work_callback(struct work_struct *work)
{
	...
	if (1 == eint_accdet_sync_flag) {
		...
		switch_set_state((struct switch_dev *)&accdet_data, cable_type);
		// joe_cheng : Z300M, Z301M headset and speaker switch (Z300M:1, Z301M:5, Z301MF:3)
		if(PROJ_ID == 0x1 || PROJ_ID == 0x5 || PROJ_ID == 0x3){
			schedule_delayed_work(&spk_hs_gpio_work, msecs_to_jiffies(800));
		}
	} else {
