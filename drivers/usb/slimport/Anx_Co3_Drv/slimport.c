...
int anx7814_i2c_status = -1;
MODULE_PARM_DESC(anx7814_i2c_status, "anx7814_i2c_status");
module_param_named(anx7814_i2c_status, anx7814_i2c_status, int, 0644);
...
#ifdef USING_HPD_FOR_POWER_MANAGEMENT
bool asus_sink_is_connected(void)
{
	bool result = false;
#ifdef CONFIG_OF
	struct anx7816_platform_data *pdata = g_pdata;
	...
	if ( (gpio_get_value(pdata->gpio_cbl_det) == 0x00) && (gpio_get_value(pdata->gpio_reset) == 0x00)) {
		sp_tx_hardware_powerdown();
		sp_tx_clean_state_machine();
		pr_info("%s asus_sink_is_connected: Downstream is disconnected\n",LOG_TAG);
		result = false;
	} else {
		result = true;
	}
...
#ifdef USING_HPD_FOR_POWER_MANAGEMENT
	//Paddy: Fix power down repeatedly issue
	if (asus_sink_is_connected())
			     ...
			     const struct i2c_device_id *id)
{
	...
	//workaround: fix power down issue during the system boot up
	anx7816_cbl_det_isr(client->irq, anx7816);

	anx7814_i2c_status = 0;
	pr_info("%s %s : anx7814_i2c_status = %d\n", LOG_TAG, __func__, anx7814_i2c_status);
