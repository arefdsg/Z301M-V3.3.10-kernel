...
void asus_anx7428_power_control(bool on);
void asus_anx7428_power_control_vcon(bool on);
static bool otg_pre_status;
...
int anx7428_i2c_status = -1;
MODULE_PARM_DESC(anx7428_i2c_status, "anx7428_i2c_status");
module_param_named(anx7428_i2c_status, anx7428_i2c_status, int, 0644);
...
int check_anx7428_status(void){
	...
	for (retry_count = 0; retry_count < 3; retry_count++) {
		...
		for (i = 0; i < OHIO_OCM_LOADING_TIME; i++) {
			/*Interface work? */
			ret = OhioReadReg(0x02);
			ret2 = OhioReadReg(0x03);
			pr_info("%s: check_anx7428_status: 02h: 0x%02x, 03h:0x%02x\n", LOG_TAG, ret, ret2);
			if(ret >= 0 && ret2 >=0){
				return 0;
			}
			mdelay(1);
...
void ohio_power_standby(void)
{
	...
	mdelay(1);
	asus_anx7428_power_control_vcon(0);
...
void ohio_hardware_poweron(void)
{
	...
	for (retry_count = 0; retry_count < 3; retry_count++) {
		...
		#endif
		asus_anx7428_power_control_vcon(1);
		/*power on pin enable */
		gpio_set_value(pdata->gpio_p_on, 1);
		mdelay(15);
...
void asus_anx7428_power_control(bool on){
	...
	pr_info("%s :asus_anx7428_power_control: enable = %d +++", LOG_TAG, on);
...
void asus_anx7428_power_control_vcon(bool on){
	...
	pr_info("%s :asus_anx7428_power_control_vcon: enable = %d +++", LOG_TAG, on);

	if (on) {
		gpio_set_value(pdata->gpio_5v_en, 1);
	} else{
		gpio_set_value(pdata->gpio_5v_en, 0);
	}
...
void ohio_vbus_control(bool value)
{
	...
	asus_anx7428_power_control(value);
...
#endif
	gpio_free(ohio->pdata->gpio_5v_en);
}
...
static int ohio_init_gpio(struct ohio_data *ohio)
{
	...
	ret = gpio_request(ohio->pdata->gpio_5v_en, "ohio_5v_en");
	if (ret) {
		pr_err("%s : failed to request gpio %d\n", __func__,
		       ohio->pdata->gpio_5v_en);
		goto err5;
	}
	gpio_direction_output(ohio->pdata->gpio_5v_en, 0);
...
#endif
	pdata->gpio_5v_en =
	    of_get_named_gpio_flags(np, "analogix,dp-5vsus-en-gpio", 0, NULL);

	pr_info("%s gpio p_on : %d, reset : %d,  gpio_cbl_det %d\n",
		LOG_TAG, pdata->gpio_p_on,
		pdata->gpio_reset, pdata->gpio_cbl_det);

	pr_info("%s gpio_5v_en : %d\n",	LOG_TAG, pdata->gpio_5v_en);
			  ...
			  const struct i2c_device_id *id)
{
	...
	anx7428_i2c_status = check_anx7428_status();
	pr_info("%s %s : anx7428_i2c_status = %d\n", LOG_TAG, __func__, anx7428_i2c_status);

	/*when probe ohio device, enter standy mode */
	ohio_power_standby();

	//workaround: fix dongle plugged before kernel driver probe
	ohio_cbl_det_isr(client->irq, ohio);
...
static int __init ohio_init(void)
{
	...
	int hw_id_1 = gpio_get_value(899);//GPIO30
	int hw_id_2 = gpio_get_value(900);//GPIO31

	pr_info("%s: hw_id_1=%d, hw_id_2=%d\n",__func__ ,hw_id_1,hw_id_2);
	if ((hw_id_1 == 0) && (hw_id_2 == 0)){
		pr_info("%s: driver off\n",__func__);
		return 0;
	}
